...
const (
	FirstError  = "Cтрока не является математической операцией"
	SecondError = "Формат математической операции не удовлетворяет заданию: два операнда и один оператор (+, -, /, *)"
	ThirdError  = "Используются одновременно разные системы счисления"
	FourthError = "В римской системе счисления нет отрицательных чисел"
	FifthError  = "В римской системе счисления нет числа 0"
	SixthError  = "Калькулятор умеет работать только с арабскими целыми числами или римскими цифрами от 1 до 10 включительно"
)
...
func main() {
	fmt.Println("Введите операцию, например: 1 + 2")
	reader := bufio.NewReader(os.Stdin)

	for {
		console, _ := reader.ReadString('\n')
		x := strings.ReplaceAll(console, " ", "")
		base(strings.ToUpper(strings.TrimSpace(x)))
	}
}
